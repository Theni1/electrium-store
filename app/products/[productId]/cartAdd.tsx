...
import { addToCart as addToCartAction } from "@/app/action/cart";
...
export default function CartAdd({ bike }: { bike: Bike }) {
  ...
  async function addToCart(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    try {
      const orderType = bike.for_rent ? "rent" : "sell";
      const result = await addToCartAction(bike.bike_id, orderType, amount);
      ...
      setNotifInfo({
        ...
        subtotal:
          orderType === "rent"
            ? bike.rental_rate * amount
            : bike.sell_price * amount,
        quantity: amount,
        numItems: 1,
      });
    } catch (error) {
